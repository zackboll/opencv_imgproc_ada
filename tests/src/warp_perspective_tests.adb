...
package body Warp_Perspective_Tests is
   ...
   procedure Set_Identity (Transform : in out OpenCV.Core.Mat) is
   begin
      OpenCV.Core.Set_To (Transform, (others => 0.0));
      if Transform.Depth = OpenCV.Core.Float32 then
         OpenCV.Core.Float32_Access.Set (Transform, 0, 0, 1.0);
         OpenCV.Core.Float32_Access.Set (Transform, 1, 1, 1.0);
         OpenCV.Core.Float32_Access.Set (Transform, 2, 2, 1.0);
      else
   ...
   function Projective_X_Float32 return OpenCV.Core.Mat is
      Transform : OpenCV.Core.Mat := Identity_Float32;
   begin
      OpenCV.Core.Float32_Access.Set (Transform, 2, 0, 0.5);
      return Transform;
   end Projective_X_Float32;
   ...
   begin
      ...
      AUnit.Assertions.Assert
        (Destination.Rows = 3
         ...
         "identity Warp_Perspective must rebind Destination to 3x4 UInt8 C1");
         ...
         for Column in 0 .. Source.Columns - 1 loop
            AUnit.Assertions.Assert
              (OpenCV.Core.UInt8_Access.Get (Destination, Row, Column)
               ...
               "identity Warp_Perspective must preserve unique UInt8 pixels");
         end loop;
      end loop;
   ...
   procedure Projective_Denominator_Selects_Donor (Test : in out Fixture) is
      pragma Unreferenced (Test);
      Source      : OpenCV.Core.Mat :=
        OpenCV.Core.Create (1, 4, (OpenCV.Core.UInt8, 1));
      Transform   : constant OpenCV.Core.Mat := Projective_X_Float32;
      Destination : OpenCV.Core.Mat;
   begin
      ...
      AUnit.Assertions.Assert
        (OpenCV.Core.UInt8_Access.Get (Destination, 0, 0) = 10,
         "projective x=0 must sample source column 0");
      AUnit.Assertions.Assert
        (OpenCV.Core.UInt8_Access.Get (Destination, 0, 2) = 20,
         "projective x=2 must sample source column 1 via the third row");
      AUnit.Assertions.Assert
        (OpenCV.Core.UInt8_Access.Get (Destination, 0, 3) = 20,
         "projective x=3 must remain in source column 1");
   end Projective_Denominator_Selects_Donor;
   ...
   begin
      ...
      AUnit.Assertions.Assert
        (Linear_Out.Depth = OpenCV.Core.Float32
         ...
         "Linear Warp_Perspective must preserve Float32 C1");
      ...
      begin
         ...
         AUnit.Assertions.Assert
           (Destination.Rows = 2
            ...
            "identity Warp_Perspective must preserve each supported"
            & " Source type");
      end Check;
   ...
   begin
      ...
      AUnit.Assertions.Assert
        (Destination.Depth = OpenCV.Core.UInt8
         ...
         "C3 Warp_Perspective must remain UInt8 C3");
   ...
   begin
      ...
      AUnit.Assertions.Assert
        (Destination.Rows = 5
         ...
         and then Destination.Depth = OpenCV.Core.UInt8
         and then Destination.Channels = 1,
         "Warp_Perspective must rebind Destination to the requested"
         & " Output_Size");
   ...
   procedure Rejects_Invalid_Public_Inputs (Test : in out Fixture) is
      ...
      Affine_Xform : constant OpenCV.Core.Mat :=
      ...
      begin
         ...
         OpenCV.Core.Float64_Access.Set (Inf_Xform, 2, 2, Value);
         OpenCV.Image_Processing.Warp_Perspective
   ...
   begin
      ...
      Assert_Raises_OpenCV_Error
        (Empty_Src'Access, "Warp_Perspective must reject an empty Source");
      Assert_Raises_OpenCV_Error
        (Three_D_Src'Access, "Warp_Perspective must reject a 3-D Source");
      Assert_Raises_OpenCV_Error
        (Int32_Src'Access, "Warp_Perspective must reject Int32");
      Assert_Raises_OpenCV_Error
        (Five_Ch'Access, "Warp_Perspective must reject more than 4 channels");
      Assert_Raises_OpenCV_Error
        (Empty_Transform'Access,
         "Warp_Perspective must reject an empty Transform");
      Assert_Raises_OpenCV_Error
        (Three_D_Transform'Access,
         "Warp_Perspective must reject a 3-D Transform");
      Assert_Raises_OpenCV_Error
        (Affine_Transform'Access,
         "Warp_Perspective must reject a 2x3 Transform");
      Assert_Raises_OpenCV_Error
        (Multi_Transform'Access,
         "Warp_Perspective must reject a multi-channel Transform");
      Assert_Raises_OpenCV_Error
        (UInt8_Transform'Access,
         "Warp_Perspective must reject a UInt8 Transform");
      Assert_Raises_OpenCV_Error
        (Zero_Width'Access, "Warp_Perspective must reject zero Width");
      Assert_Raises_OpenCV_Error
        (Cubic_Interp'Access, "Warp_Perspective must reject Cubic");
      Assert_Raises_OpenCV_Error
        (Wrap_Border'Access, "Warp_Perspective must reject Wrap");
      Assert_Raises_OpenCV_Error
        (Alias_Source'Access,
         "Warp_Perspective must reject Destination aliased with Source");
      Assert_Raises_OpenCV_Error
        (Alias_Transform'Access,
         "Warp_Perspective must reject Destination aliased with Transform");
      Assert_Raises_OpenCV_Error
        (Shared_Source_Data'Access,
         "Warp_Perspective must reject Destination sharing Source storage");
      Assert_Raises_OpenCV_Error
        (Nan_Transform'Access, "Warp_Perspective must reject a NaN Transform");
      Assert_Raises_OpenCV_Error
        (Inf_Transform'Access,
         "Warp_Perspective must reject an infinite Transform");
      ...
      begin
         ...
         AUnit.Assertions.Assert
           (Status = C_API.Error_Invalid_Argument,
            "malformed Warp_Perspective C ABI must return invalid argument");
         AUnit.Assertions.Assert
           (Ada.Strings.Fixed.Index (C_API.Last_Error_Message, Diagnostic)
            /= 0,
            "malformed Warp_Perspective C ABI input must identify "
            & Diagnostic);
      end Check;
      ...
      begin
         ...
         AUnit.Assertions.Assert
           (Status = C_API.Success,
            "valid Warp_Perspective C ABI call must succeed");
         AUnit.Assertions.Assert
           (Destination.Rows = 3 and then Destination.Columns = 3,
            "valid Warp_Perspective C ABI call must produce the requested"
            & " size");
      end Check_Success;
   begin
      ...
      begin
         Value := Bits_To_Float32 (NaN_Bits_32);
         OpenCV.Core.Float32_Access.Set (Nan_Xform, 2, 2, Value);
      end;
      ...
      Check (UInt8_Source, Affine_Xform, 3, 3, 0, 0, 0, "3x3");
   ...
   begin
      Result.Add_Test
        (Caller.Create
           ("Warp_Perspective identity rebinds Destination",
            Identity_Rebinds_Destination'Access));
      Result.Add_Test
        (Caller.Create
           ("Warp_Perspective nearest integer translation",
            Nearest_Integer_Translation'Access));
      Result.Add_Test
        (Caller.Create
           ("Warp_Perspective projective denominator selects donor",
            Projective_Denominator_Selects_Donor'Access));
      Result.Add_Test
        (Caller.Create
           ("Warp_Perspective inverse mapping reverses translation",
            Inverse_Mapping_Reverses_Translation'Access));
      Result.Add_Test
        (Caller.Create
           ("Warp_Perspective Linear fractional transform interpolates",
            Linear_Fractional_Transform'Access));
      Result.Add_Test
        (Caller.Create
           ("Warp_Perspective nonzero Constant_Border",
            Nonzero_Constant_Border'Access));
      Result.Add_Test
        (Caller.Create
           ("Warp_Perspective Replicate ignores Border_Value",
            Replicate_Border_Ignores_Value'Access));
      Result.Add_Test
        (Caller.Create
           ("Warp_Perspective Float64 Transform preserves UInt8 destination",
            Float64_Transform_Works'Access));
      Result.Add_Test
        (Caller.Create
           ("Warp_Perspective supports the documented source depths",
            Supported_Source_Depths'Access));
      Result.Add_Test
        (Caller.Create
           ("Warp_Perspective C3 Constant_Border transmits Scalar components",
            C3_Constant_Border_Components'Access));
      Result.Add_Test
        (Caller.Create
           ("Warp_Perspective Output_Size rebinds Destination",
            Output_Size_Rebinds_Destination'Access));
      Result.Add_Test
        (Caller.Create
           ("Warp_Perspective rejects invalid public inputs",
            Rejects_Invalid_Public_Inputs'Access));
      Result.Add_Test
        (Caller.Create
           ("Warp_Perspective C ABI rejects malformed inputs",
            C_ABI_Rejects_Malformed_Inputs'Access));
      return Result'Access;
   end Suite;

end Warp_Perspective_Tests;
